//! Runtime-level parity test for `Tensor::add` against MLX's `mx.add`.
//!
//! Drives the dispatch through the runtime API end-to-end:
//! `from_slice` two operands, schedule `add`, `eval()`, read host
//! bytes. Two cases mirror the kernels-crate test in
//! `crates/cider-press-kernels/tests/binary_parity.rs`:
//!
//! - **vv** — `[1, S, H] + [1, S, H]` bf16. Output should be bit-exact
//!   identical to MLX since bf16 add is a single rounded op and the
//!   runtime threads the same MLX kernel.
//! - **broadcast** — `[1, S, H] + [H]`. The runtime's `Tensor::binary`
//!   inserts a `broadcast_to` view on the rhs which makes the
//!   dispatcher pick `g3_Addbfloat16`.
//!
//! Like the kernels-crate parity test, fixtures are generated at test
//! time via `uv run scripts/dump_mlx_op.py add`; CI installs uv via
//! astral-sh/setup-uv. See that file's module docstring for the
...
fn tempdir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!("cider-press-runtime-binary-parity-{pid}-{nanos}"));
