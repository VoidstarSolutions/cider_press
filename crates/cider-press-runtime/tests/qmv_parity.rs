//! Runtime-level parity test for `QuantizedWeight::matvec` against
//! MLX's `mx.quantized_matmul`.
//!
...
//! `QuantizedWeight`, build an activation tensor, schedule `matvec`,
//! `eval()`, and read the result back.
//!
...
fn qmv_through_runtime_matches_mlx() {
    ...
    let y = qw.matvec(&x).expect("schedule matvec");
    assert!(!y.is_materialized(), "matvec is lazy");
