...
2. **head_dim-256 Gated-Attention SDPA** specialization (vendored steel attn is
   ...
   **output gate** (q_proj emits `2 × (H_q · D_h)` — 8192 on the 4B, 12288 on
   the 27B — split **per-head** into query‖gate: reshape `[…, H_q, 2·D_h]` then
   split last axis, not a flat block split). Not plain attention.
...
**Resolved in Phase 2 (2026-06-14):**
- ✓ **mRoPE 1-D collapse** for text (sections `[11,11,10]`) — confirmed:
  `rope_type=default` collapses mRoPE to plain 1-D partial RoPE for text, so no
  multi-section position handling is needed.
